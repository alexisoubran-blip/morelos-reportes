(() => {
  ...
  function filteredContent(range){
    return DATA.social.content.filter(r=>inRange(parseDate(r.date),range) && (state.campaign==='all'||r.campaign_group===state.campaign));
  }
  function filteredFollows(range){
    if(state.campaign!=='all') return [];
    return DATA.social.follows_daily.filter(r=>inRange(parseDate(r.date),range));
  }
  ...
  function renderInvestmentTime(range, paid){
    ...
    $('investment-time-chart').innerHTML=barChart(labels,vals,{money:true,caption:'Google Ads · inversión semanal disponible. Meta requiere export temporal para entrar a esta gráfica.'});
  }
  ...
  function renderCampaignSplit(paid){
    ...
    $('campaign-split').innerHTML=rows.map(r=>`<div class="rank-row"><div class="rank-label">${esc(r.group)}</div><div class="rank-track"><div class="rank-fill" style="width:${r.spend/max*100}%"></div></div><div class="rank-value">${money(r.spend)}</div></div>`).join('');
  }
  ...
  function renderPlatformPerformance(paid){
    ...
    $('platform-performance').innerHTML=cards.map((c,i)=>`<div class="platform-card ${c.value==null?'muted-platform':''}"><div class="platform-top"><h4>${c.name}</h4><span class="status-chip ${c.value==null?'muted':''}">${label}</span></div><div class="platform-value">${c.value==null?'—':fmt(c.value)}</div><p>${esc(c.note)}</p></div>`).join('');
  }
  ...
  function renderSocial(range){
    ...
    setText('social-follows',state.campaign==='all'?compact(followCount):'—');
    setText('social-views',compact(views));
    setText('social-interactions',compact(interactions));
    setText('social-er',reach?pct(interactions/reach):'—');
  ...
  function renderSocialChart(range,content,follows){
    const metric=state.socialMetric, grain=grainForTrend(), map=new Map();
    if(metric==='follows'){
      if(state.campaign!=='all'){ $('social-chart').innerHTML=empty('Follows no tiene dimensión de campaña en el export.'); return; }
      follows.forEach(r=>{const k=bucketKey(parseDate(r.date),grain);map.set(k,(map.get(k)||0)+r.follows)});
    } else {
      content.forEach(r=>{const k=bucketKey(parseDate(r.date),grain);map.set(k,(map.get(k)||0)+(metric==='views'?r.views:r.interactions))});
    }
    const rows=[...map.entries()].sort((a,b)=>a[0].localeCompare(b[0]));
    if(!rows.length){$('social-chart').innerHTML=empty('Sin registros sociales para este filtro.');return}
    $('social-chart').innerHTML=barChart(rows.map(x=>bucketLabel(x[0],grain)),rows.map(x=>x[1]),{caption:metric==='follows'?'Follows obtenidos por fecha.':'Métrica Lifetime agrupada por fecha de publicación.'});
  }
  ...
  function renderTrafficSources(){
    ...
    $('traffic-sources').innerHTML=rows.map(r=>`<div class="rank-row"><div class="rank-label">${esc(r.channel)}</div><div class="rank-track"><div class="rank-fill" style="width:${r.value/max*100}%"></div></div><div class="rank-value">${compact(r.value)}</div></div>`).join('');
  }
  function renderPages(){
    const rows=(DATA.analytics.pages||[]).slice(0,7); const max=Math.max(...rows.map(r=>r.views),1);
    $('top-pages').innerHTML=rows.map(r=>`<div class="rank-row"><div class="rank-label" title="${esc(r.page)}">${esc(r.page)}</div><div class="rank-track"><div class="rank-fill" style="width:${r.views/max*100}%"></div></div><div class="rank-value">${compact(r.views)}</div></div>`).join('');
  }
  ...
  function renderInsights(paid,social,analytics){
    ...
    if(social.views>0) cards.push({title:'Social mantiene una base de lectura por contenido',body:`El contenido publicado en el periodo acumula ${compact(social.views)} views Lifetime y ${compact(social.interactions)} interacciones.`});
    if(social.followCount>0 && state.campaign==='all') cards.push({title:'Crecimiento social medible por fecha',body:`Facebook + Instagram registran ${integer(social.followCount)} nuevos follows dentro del periodo seleccionado.`});
  ...
  function barChart(labels,values,opts={}){
    ...
    const bars=values.map((v,i)=>{const h=v/max*ih,x=p.l+i*step+(step-bw)/2,y=p.t+ih-h;return `<rect class="bar ${i%2?'alt':''}" x="${x}" y="${y}" width="${bw}" height="${h}" rx="4"/><text x="${x+bw/2}" y="${H-18}" text-anchor="middle">${esc(labels[i])}</text>`}).join('');
    return `<svg class="chart-svg" viewBox="0 0 ${W} ${H}" role="img">${grid}<line class="axis" x1="${p.l}" y1="${p.t+ih}" x2="${W-p.r}" y2="${p.t+ih}"/>${bars}</svg>${opts.caption?`<div class="chart-legend"><span><i class="legend-dot"></i>${esc(opts.caption)}</span></div>`:''}`;
  }
  ...
  async function init(){
    try{
      ...
      $('reset-filters').addEventListener('click',()=>{Object.assign(state,{mode:'months',week:'2026-06-01',month:'2026-07',fromMonth:'2026-06',toMonth:'2026-07',campaign:'all',channel:'all'});$('campaign-filter').value='all';$('channel-filter').value='all';renderAll()});
