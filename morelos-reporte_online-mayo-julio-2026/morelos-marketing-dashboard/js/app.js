(() => {
  ...
  function renderSocialChart(content,follows){
    const metric=state.socialMetric,grain=grainForTrend(),source=metric==='follows'?follows:content;
    if(!source.length){$('social-chart').innerHTML=empty(state.socialPlatform==='TikTok'?'TikTok sin fuente integrada.':'Sin registros sociales para este filtro.');return}
    const valueFor=r=>metric==='follows'?r.follows:metric==='views'?r.views:r.interactions;
    const caption=metric==='follows'?'Nuevos seguidores obtenidos por fecha.':metric==='views'?'Business Suite por plataforma.':'Interacciones por plataforma.';
    if(state.socialPlatform!=='all'){
      const map=new Map();source.forEach(r=>{const k=bucketKey(parseDate(r.date),grain);map.set(k,(map.get(k)||0)+(valueFor(r)||0))});
      const rows=[...map.entries()].sort((a,b)=>a[0].localeCompare(b[0]));
      $('social-chart').innerHTML=barChart(rows.map(x=>bucketLabel(x[0],grain)),rows.map(x=>x[1]),{color:PLATFORM_COLORS[state.socialPlatform],caption});return;
    }
    const pm=new Map(),keys=new Set();source.forEach(r=>{const k=bucketKey(parseDate(r.date),grain);keys.add(k);if(!pm.has(r.platform))pm.set(r.platform,new Map());const m=pm.get(r.platform);m.set(k,(m.get(k)||0)+(valueFor(r)||0))});
    const ordered=[...keys].sort(),platforms=['Facebook','Instagram'].filter(p=>pm.has(p)),series=platforms.map(p=>({name:p,color:PLATFORM_COLORS[p],values:ordered.map(k=>pm.get(p).get(k)||0)}));
    $('social-chart').innerHTML=groupedBarChart(ordered.map(k=>bucketLabel(k,grain)),series,{caption:metric==='views'?'La gráfica conserva raw por plataforma; el KPI combinado usa el rollup deduplicado del Data Hub.':caption});
  }
  ...
  async function init(){
    try{
      const [dataRes,soiRes,followersRes]=await Promise.all([
        fetch('./data/dashboard-data.json',{cache:'no-store'}),
        fetch('./config/soi-investment.json',{cache:'no-store'}),
        fetch('./data/social-followers.json',{cache:'no-store'})
      ]);
      if(!dataRes.ok||!soiRes.ok||!followersRes.ok)throw new Error('No se pudieron cargar las fuentes.');
      const [data,soi,followers]=await Promise.all([dataRes.json(),soiRes.json(),followersRes.json()]);
      DATA=data;SOI=soi;DATA.social=DATA.social||{};DATA.social.follows_daily=followers;
      ...
      $('social-platform-tabs').addEventListener('click',e=>{const b=e.target.closest('button[data-platform]');if(!b)return;state.socialPlatform=b.dataset.platform;renderAll()});
      $('reset-filters').addEventListener('click',()=>{Object.assign(state,{mode:'months',week:'2026-06-01',month:'2026-07',fromMonth:'2026-06',toMonth:'2026-07',campaign:'all',channel:'all',socialPlatform:'all'});$('campaign-filter').value='all';$('channel-filter').value='all';renderAll()});
      bindTabs('performance-tabs','performanceMetric');bindTabs('social-tabs','socialMetric');bindTabs('analytics-tabs','analyticsMetric');renderAll();
    }catch(err){console.error(err);document.body.innerHTML=`<main class="page-shell"><div class="coverage-banner"><strong>No se pudieron cargar las fuentes del dashboard.</strong></div></main>`}
  }
  init();
})();
