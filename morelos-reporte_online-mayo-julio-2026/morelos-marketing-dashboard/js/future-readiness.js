(() => {
  ...
  function activeRange() {
    ...
    if (mode === 'week') {
      ...
      return {start,end,label:`${iso(start)} → ${iso(end)}`};
    }
    if (mode === 'month') {
      const value = document.getElementById('month-select')?.value || '2026-07';
      const [y,m] = value.split('-').map(Number);
      return {start:new Date(y,m-1,1,12),end:new Date(y,m,0,12),label:value};
    }
    ...
    return {start:new Date(fy,fm-1,1,12),end:new Date(ty,tm,0,12),label:from===to?from:`${from} → ${to}`};
  }
  ...
  function refreshFutureUi() {
    ensureFutureChannels(); ensurePaidPlaceholders(); ensureSocialTikTokTab(); renderReviews();
  }
